...
void SimplePlanningSimulator::initialize_vehicle_model(const std::string & vehicle_model_type_str)
{
  ...
  const double acc_lim = declare_parameter("acc_lim", 7.0);
  const double brake_lim = declare_parameter("brake_lim", 7.0);
  const double vel_rate_lim = acc_lim;  // For models without separate acc/brake limits
  const double acc_rate_lim = declare_parameter("acc_rate_lim", 5.0);
  const double brake_rate_lim = declare_parameter("brake_rate_lim", 10.0);
  ...
  const double acc_accuracy_error = declare_parameter("acc_accuracy_error", 0.0);
  ...
  const double acc_offset = declare_parameter("acc_offset", 0.0);
  const double brake_offset = declare_parameter("brake_offset", 0.0);
  const double acc_resolution = declare_parameter("acc_resolution", 0.0);
  ...
  const double vel_sensor_resolution = declare_parameter("vel_sensor_resolution", 0.0);
  const double vel_sensor_noise_stddev = declare_parameter("vel_sensor_noise_stddev", 0.0);
  const int vel_sensor_noise_seed = declare_parameter("vel_sensor_noise_seed", 42);
  const double vel_sensor_accuracy_error = declare_parameter("vel_sensor_accuracy_error", 0.0);
  const double vel_sensor_offset = declare_parameter("vel_sensor_offset", 0.0);
  ...
  if (vehicle_model_type_str == "IDEAL_STEER_VEL") {
    ...
    vehicle_model_ptr_ = std::make_shared<SimModelDelaySteerAccGearedWoFallGuard>(
      vel_lim, acc_lim, brake_lim, acc_rate_lim, brake_rate_lim, steer_lim, steer_rate_lim, wheelbase, timer_sampling_time_ms_ / 1000.0,
