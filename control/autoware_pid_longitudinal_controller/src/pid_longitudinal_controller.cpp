  ...
  logger_(node.get_logger().get_child("longitudinal_controller"))
{
  ...
  // parameters for drive state
  {
    ...
    m_brake_keeping_stop_dist = node.declare_parameter<double>("brake_keeping_stop_dist", 0.5); // TODO: Temporary default value. Remove this and add the parameter to autoware_launch yaml.
  }
  ...
  const ControlData & control_data, const Motion & target_motion) const
{
  ...
  if (control_data.stop_dist <= m_brake_keeping_stop_dist) {
    if (target_motion.acc > m_brake_keeping_acc) {
